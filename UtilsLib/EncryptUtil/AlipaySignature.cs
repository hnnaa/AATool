...
namespace UtilsLib.EncryptUtil
{
	...
	public static bool RSACheckV2(IDictionary<string, string> parameters, string publicKeyPem, string charset, string signType, bool keyFromFile)
        ...
        public static byte[] RSAEncrypt(RSACryptoServiceProvider rsaCsp,byte[] data)
        {
            int maxBlockSize = rsaCsp.KeySize / 8 - 11; //加密块最大长度限制
            if (data.Length <= maxBlockSize)
            {
                byte[] cipherbytes = rsaCsp.Encrypt(data, false);
                return cipherbytes;
            }
            ...
            using (MemoryStream crypStream = new MemoryStream())
            {
                ...
                while (blockSize > 0)
                {
                    Byte[] toEncrypt = new Byte[blockSize];
                    Array.Copy(buffer, 0, toEncrypt, 0, blockSize);
                    Byte[] cryptograph = rsaCsp.Encrypt(toEncrypt, false);
                    crypStream.Write(cryptograph, 0, cryptograph.Length);
                    blockSize = plaiStream.Read(buffer, 0, maxBlockSize);
                }
        ...
        public static byte[] RSADecrypt(RSACryptoServiceProvider rsaCsp, byte[] data)
        {
            int maxBlockSize = rsaCsp.KeySize / 8; //解密块最大长度限制
            if (data.Length <= maxBlockSize)
            {
